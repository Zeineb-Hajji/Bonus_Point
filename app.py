...
# Parameters in main area with horizontal layout
st.subheader("⚙️ Clustering Parameters")

col1, col2, col3, col4 = st.columns(4)

with col1:
    st.markdown("**1. Value Perspective**")
    n_clusters_value = st.slider("Clusters (Value)", 2, 10, 4, key="value")

with col2:
    st.markdown("**2. Behaviour Perspective**")
    n_clusters_behaviour = st.slider("Clusters (Behaviour)", 2, 10, 3, key="behaviour")

with col3:
    st.markdown("**3. Demographic Perspective**")
    n_clusters_demo = st.slider("Clusters (Demographics)", 2, 10, 6, key="demo")

with col4:
    st.markdown("**4. Merged Perspective**")
    n_final_clusters = st.slider("Final Clusters", 2, 12, 6, key="final")
